...
namespace QGEP.Tests
{
    public abstract class QGEPTestBase : AbpIntegratedTestBase<QGEPTestModule>
    {
        ...
        protected void UsingDbContext(Action<QGEPDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<QGEPDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<QGEPDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<QGEPDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<QGEPDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<QGEPDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<QGEPDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<QGEPDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<QGEPDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<QGEPDbContext, Task<T>> func)
        {
            T result;
